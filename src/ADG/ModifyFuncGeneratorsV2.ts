...
import {
    findParentTagsByTagIdV2,
    findTaskByTaskIdV2,
    findTagByTagIdV2,
} from './FindItemsV2.js'
import {
    addOrphanTasktoTaskListV2,
    deleteEntireTaskV2,
    generateEmptyTaskV2,
    insertTaskInstanceIntoTaskV2,
    insertTaskInstanceIntoTagV2,
    removeTaskInstanceFromTagV2,
} from './ModifyTaskTopicAdgElementsV2.js';

// Task moving
// sources: 1. task in tag, 2. childtask in task
// targets: A.task in (other) tag, B. childtask in (other) task
// So in total:
// 1A. Move task in tag to task in other tag
// 1B. Move task in tag to childtask in other task
// 2A. Move childtask in task to task in some tag
// 2B. Move childtask in task to childtask in other task

const findTaskInSubTaskTreeV2 = (taskMap, rootTaskId, childTaskId) => {
    if (rootTaskId === childTaskId) { return true; }
    let rootTask = findTaskByTaskIdV2(taskMap, rootTaskId)
}
