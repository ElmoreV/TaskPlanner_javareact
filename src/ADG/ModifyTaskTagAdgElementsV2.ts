...
const insertTaskInstanceIntoTagV2 = (tagTaskMap: TagTasksMap, taskId: number, tagId: number, tagOrderIndex?: number) => {
    console.info(`Insert task instance with task.id: ${taskId} into topic with id: ${tagId} at view index ${tagOrderIndex}`)
...
// Returns a shallow copy with the changed tasks 
const insertTaskInstanceIntoTaskV2 = (taskMap: TaskMap, subTaskId: number, superTaskId: number) => {
...
const removeTaskInstanceFromTagV2 = (tagTaskMap: TagTasksMap, taskId: number, tagId: number) => {
    console.info(`Removing task instance with task.id: ${taskId} from topic with id: ${tagId}`)
    let newTagTaskMap = { ...tagTaskMap }
...
// Assumes tha taskId exists in the tasks
const removeTaskInstanceFromTaskV2 = (taskMap: TaskMap, taskId: number, superTaskId: number) => {
...
const deleteEntireTaskV2 = (taskMap: TaskMap, tagTaskMap: TagTasksMap, taskId: number) => {
    let newTaskMap = { ...taskMap }
