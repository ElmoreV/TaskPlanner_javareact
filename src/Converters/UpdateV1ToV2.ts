...
export const ensureRoundtripStability = (tasks, topics) => {
    ...
    allTaskIds.forEach((taskId) => {
        ...
        if (refTask.name !== reviewTask.name) {
            console.log("Task names are different: " + refTask.name + " vs " + reviewTask.name)
        }
        if (refTask.topics.length !== reviewTask.topics.length) {
            console.log("Task topics are different lenghts: " + refTask.topics.length + " vs " + reviewTask.topics.length)
        }
        if (refTask.topics.length !== reviewTask.topics.length) {
            console.log("Task topics are different lenghts: " + refTask.topics.length + " vs " + reviewTask.topics.length)
        } else if (!refTask.topics.every(value => reviewTask.topics.includes(value))) {
            console.log("Task topics have different elements: " + refTask.topics + " vs " + reviewTask.topics)
        }


        if (refTask.topicViewIndices.length !== reviewTask.topicViewIndices.length) {
            console.log("Task topics are different lenghts: " + refTask.topics.length + " vs " + reviewTask.topics.length)
        }
        ...
        // }
        if (refTask.subTaskIds && refTask.subTaskIds.length !== reviewTask.subTaskIds.length) {
            console.log("Task subTaskIds are different lenghts: " + refTask.subTaskIds.length + " vs " + reviewTask.subTaskIds.length)
        } else if (!refTask.subTaskIds.every(value => reviewTask.subTaskIds.includes(value))) {
            console.log("Task subTaskIds have different elements: " + refTask.subTaskIds + " vs " + reviewTask.subTaskIds)
        }
        if ((refTask.thisWeek !== reviewTask.thisWeek && refTask.thisWeek !== undefined)
            || refTask.thisWeek === undefined && reviewTask.thisWeek === true) {
            console.log("Task thisWeek are different: " + refTask.thisWeek + " vs " + reviewTask.thisWeek)
        }
        if (refTask.weekOrderIndex !== reviewTask.weekOrderIndex) {
            console.log("Task weekOrderIndex are different: " + refTask.weekOrderIndex + " vs " + reviewTask.weekOrderIndex)
        }
        if (refTask.finishStatus !== undefined && refTask.finishStatus !== reviewTask.finishStatus) {
            console.log("Task finishStatus are different: " + refTask.finishStatus + " vs " + reviewTask.finishStatus)
        } else if (refTask.finishStatus === undefined && reviewTask.finishStatus !== undefined) {
            if (refTask.completed && reviewTask.finishStatus !== FinishedState.Completed) {
                console.log("Task finishStatus are different: " + refTask.finishStatus + " vs " + reviewTask.finishStatus)
            }
        }
        ...
        if (refTask.repeated !== reviewTask.repeated) {
            console.log("Task repeated are different: " + refTask.repeated + " vs " + reviewTask.repeated)
        }
        if (refTask.scheduled !== reviewTask.scheduled) {
            console.log("Task scheduled are different: " + refTask.scheduled + " vs " + reviewTask.scheduled)
        }
        if (refTask.unfolded !== reviewTask.unfolded) {
            console.log("Task unfolded are different: " + refTask.unfolded + " vs " + reviewTask.unfolded)
        }
        if (refTask.dueTime !== reviewTask.dueTime) {
            console.log("Task dueTime are different: " + refTask.dueTime + " vs " + reviewTask.dueTime)
        }
        if (refTask.transitiveDueTime !== reviewTask.transitiveDueTime) {
            console.log("Task transitiveDueTime are different: " + refTask.transitiveDueTime + " vs " + reviewTask.transitiveDueTime)
        }
        if (refTask.lastFinished !== reviewTask.lastFinished) {
            console.log("Task lastFinished are different: " + refTask.lastFinished + " vs " + reviewTask.lastFinished)
        }
    }
    )
