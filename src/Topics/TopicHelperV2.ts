...
const getFreeTaskIdV2 = (taskMap: TaskMap) => {
    return 1 + Object.keys(taskMap).reduce((maxId, taskId) => Math.max(maxId, Number(taskId)), 0);
}
...
const disconnectChildTagsByIdV2 = (tagMap: TagMap, tagId: number) => {
    ...
    Object.values(tagMap)
        .forEach((tag) => {
            tag.childTagIds = tag.childTagIds.filter(childTagId => childTagId !== tagId)
            tag.parentTagIds = tag.parentTagIds.filter(parentTagId => parentTagId !== tagId)
        })
}
...
const getAllDescendantTagsV2 = (tagMap: TagMap, tagId: number) => {
    ...
    return [tag, ...tag.childTagIds.flatMap(childTagId => getAllDescendantTagsV2(tagMap, childTagId))]
}
